...
class DiceStage(QWidget):
    """Polyhedral dice rendered from the supplied model reference and simulated by Jolt."""
    ...
    def roll(self):
        ...
        pos=(random.uniform(-.9,-.48),random.uniform(-.28,.28),random.uniform(3.8,4.5))
        if self.die_type==6:
            d6_half=0.335
            self.die_body=self.world.create_body(
                pos=pos,rot=quat,size=(d6_half,)*3,
                shape=culverin.SHAPE_BOX,motion=culverin.MOTION_DYNAMIC,
                mass=1.,friction=.58,restitution=.28,ccd=True)
        else:
            self.die_body=self.world.create_convex_hull(
                pos=pos,rot=quat,points=np.asarray(self.vertices,dtype=np.float32),
                motion=culverin.MOTION_DYNAMIC,mass=1.,
                friction=.58,restitution=.28,ccd=True)

        self.world.apply_impulse(
            self.die_body,random.uniform(.95,1.45),
            random.uniform(-.30,.36),random.uniform(-.25,-.05))
        self.world.apply_angular_impulse(
            self.die_body,random.uniform(.045,.080),
            random.uniform(-.080,-.045),random.uniform(.028,.058))

        self.physics_accumulator=0.
        self.sleep_time=0.
        self.unstable_time=0.
        self.last_tick=time.perf_counter()
        self.animating=True
        self._timer.start()
    ...
    def _tick(self):
        ...
        if not pos or not quat or not lv or not av:return

        linear=math.sqrt(sum(float(v)*float(v) for v in lv))
        angular=math.sqrt(sum(float(v)*float(v) for v in av))
        flat=self._flatness(quat)
        grounded=float(pos[2])<self.die_radius*1.9

        if self.die_type==4:
            pose_ok = flat > .982
        elif self.die_type in (8,12):
            # For these rounded imported meshes, low kinetic energy + ground
            # contact is the reliable settling condition. Result selection is
            # still computed from the nearest logical face direction afterward.
            pose_ok = True
        else:
            pose_ok = flat > .987

        if grounded and linear<.060 and angular<.19 and pose_ok:
            self.sleep_time+=frame_dt
        else:
            self.sleep_time=max(0.,self.sleep_time-frame_dt*2.2)

        if self.sleep_time>.46:
    ...
    def _physics_step(self):
        ...
        if pos and quat and lv and av:
            ...
            angular=math.sqrt(sum(float(v)*float(v) for v in av))
            flat=self._flatness(quat)
            grounded=float(pos[2])<self.die_radius*1.95
            if self.die_type==4:
                unstable_pose = flat < .972
            elif self.die_type in (8,12):
                # Jolt's convex hull already determines their stable face. Do not
                # continuously disturb a physically settled D8/D12 because of
                # imported-model normal noise.
                unstable_pose = False
            else:
                unstable_pose = flat < .980

            if grounded and unstable_pose and linear<.11 and angular<.30:
                self.unstable_time+=self.physics_dt
                if self.unstable_time>.10:
                    t=time.perf_counter()
                    self.world.apply_torque(
                        self.die_body,
                        .00018*math.sin(t*17.1),
                        .00016*math.cos(t*13.7),
                        .00007*math.sin(t*9.3))
            else:
                self.unstable_time=0.
        self.world.step(self.physics_dt)

    def _apply_transform_from_world(self):
        if self.die_mesh is None or self.die_body is None:return
        pos=self.world.get_position(self.die_body)
        quat=self.world.get_rotation(self.die_body)
        if not pos or not quat:return
...
class DicePage(QWidget):
    def __init__(self, parent=None):
        ...
        desc = QLabel(TXT(
            "当前仅开放 D6。其他骰子类型的代码仍保留在程序中，之后继续完善后再重新显示。",
            "Only D6 is currently available. Other die types remain in the code and will be shown again after further refinement."
        ))
        ...
        self.die_buttons = {}
        for sides in (6,):
            b = QPushButton(f"D{sides}")
            b.setObjectName("chipButton")
            b.setCheckable(True)
            b.setCursor(Qt.PointingHandCursor)
            b.setMinimumWidth(64)
            b.clicked.connect(lambda checked, s=sides: self._choose_die(s))
            selector.addWidget(b)
            self.die_buttons[sides] = b
        ...
        bottom = QHBoxLayout()
        self.status = QLabel(TXT("D6 已选择 · 点击投掷", "D6 selected · press Roll"))
        ...
        self.die_buttons[6].setChecked(True)

        if not OPENGL_3D_AVAILABLE or not JOLT_AVAILABLE:
            self.roll_button.setEnabled(False)
            for b in self.die_buttons.values():
                b.setEnabled(False)
    ...
    def _choose_die(self, sides):
        if self.stage.animating:
            return
        for s, b in self.die_buttons.items():
            b.setChecked(s == sides)
        self.stage.set_die_type(sides)
        self.status.setText(TXT(
            f"D{sides} 已选择 · 点击投掷",
            f"D{sides} selected · press Roll"
        ))
    ...
    def _roll(self):
        ...
        for b in self.die_buttons.values():
            b.setEnabled(False)
        self.status.setText(TXT(
            f"D{self.stage.die_type} 正在弹跳…",
            f"D{self.stage.die_type} is rolling…"
        ))
        self.stage.roll()
    ...
    def _finished(self, value):
        ...
        for b in self.die_buttons.values():
            b.setEnabled(True)
        self.status.setText(TXT(
            f"D{self.stage.die_type} 结果：{value}",
            f"D{self.stage.die_type} result: {value}"
        ))
